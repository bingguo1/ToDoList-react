...
class App extends React.Component {
  constructor(props) {
    ...
    if(window.localStorage.getItem("ToDoList_storage")){
        ...
        this.state.todolist.forEach( todo=>{
          todo.addTime=new Date(todo.addTime);
      ...
      this.state.todolist.push({
        ...
        completed: false,
        dueDate: null,
        index: hashCode("Prepare for tomorrow")
      });
  ...
  saveToDo2LocalStorage=()=>{
    window.localStorage.setItem("ToDoList_storage", JSON.stringify(this.state.todolist));
}
componentDidMount(){
    window.addEventListener('beforeunload', this.saveToDo2LocalStorage);
  }

  componentWillUnmount() {
      this.saveToDo2LocalStorage();
      window.removeEventListener('beforeunload', this.saveToDo2LocalStorage); // remove the event handler for normal unmounting
  }
  componentDidUpdate(){
  ...
  receiveNewToDo = (todo, dueDate) => {
    const newToDo = {
      ...
      completed: false,
      index: hashCode(todo) 
      ///// do not use length here, if you delete a index and then add a new one, 
      ////// then you get a same index for two todos
    };
    this.setState({ todolist: [ ...this.state.todolist, newToDo] });
  }

  switchCompletedStatus = (index) => {
    let i = this.state.todolist.findIndex(todo=>todo.index===index);
    let newToDoList = [...this.state.todolist];
    let newToDo = { ...newToDoList[i], completed: !newToDoList[i].completed };
  ...
  editThisToDoItem = (index) => {
    this.setState({ contentInEditing: this.state.todolist.find(todo=> todo.index===index).content });
